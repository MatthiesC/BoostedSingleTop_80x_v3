 ...
 * another option is to use the BoostedSingleTopGenProducer (see below), which writes the
 * result to the event container, where it can be accessed later.
 */
class BoostedSingleTopGen {
 public:
   ...
   */
  explicit BoostedSingleTopGen(const uhh2::Event & event);
...
class BoostedSingleTopGenProducer: public uhh2::AnalysisModule {
 public:
  explicit BoostedSingleTopGenProducer(uhh2::Context & ctx, const std::string & name = "BoostedSingleTopgen");
