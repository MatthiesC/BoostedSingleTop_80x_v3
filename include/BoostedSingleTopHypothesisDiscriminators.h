...
const BoostedSingleTopHypothesis * get_best_hypothesis(const std::vector<BoostedSingleTopHypothesis> & hyps, const std::string & label);


class BoostedSingleTopChi2Discriminator: public uhh2::AnalysisModule {

 public:

  BoostedSingleTopChi2Discriminator(uhh2::Context&, const std::string&);
 ...
 private:
  uhh2::Event::Handle<std::vector<BoostedSingleTopHypothesis>> h_hyps;
 ...
 * 
 * Writes a "BoostedSingleTopMatch" quality flags to the reconstruction hypotheses, which is the sum of Delta R values
 ...
 */
class BoostedSingleTopMatchDiscriminator: public uhh2::AnalysisModule {
public:
    struct cfg {
        std::string bstartotwgen_name;
        std::string discriminator_label;
        cfg(): bstartotwgen_name("BoostedSingleTopgen"), discriminator_label("Match"){}
    };
    
    BoostedSingleTopMatchDiscriminator(uhh2::Context & ctx, const std::string & rechyps_name, const cfg & config = cfg());
    virtual bool process(uhh2::Event & event) override;
    
private:
    
    uhh2::Event::Handle<std::vector<BoostedSingleTopHypothesis>> h_hyps;
    uhh2::Event::Handle<BoostedSingleTopGen> h_bstartotwgen;
    cfg config;
};
