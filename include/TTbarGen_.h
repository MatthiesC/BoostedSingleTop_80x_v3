 ...
 * another option is to use the TTbarGen_Producer (see below), which writes the
...
public:
   ...
   */
  explicit TTbarGen_(const std::vector<GenParticle> & genparts, bool throw_on_failure = true);
...
class TTbarGen_Producer: public uhh2::AnalysisModule {
public:
    explicit TTbarGen_Producer(uhh2::Context & ctx, const std::string & name = "ttbargen", bool throw_on_failure = true);
