...
const BoostedSingleTopHypothesis * get_best_hypothesis(const std::vector<BoostedSingleTopHypothesis> & hyps, const std::string & label) {
  ...
  const BoostedSingleTopHypothesis * best = nullptr;
...
BoostedSingleTopChi2Discriminator::BoostedSingleTopChi2Discriminator(Context & ctx, const std::string & rechyps_name) {

  h_hyps = ctx.get_handle<vector<BoostedSingleTopHypothesis>>(rechyps_name);
...
bool BoostedSingleTopChi2Discriminator::process(uhh2::Event& event) {
...
BoostedSingleTopMatchDiscriminator::BoostedSingleTopMatchDiscriminator(Context & ctx, const std::string & rechyps_name, const cfg & config_): config(config_){
  h_hyps = ctx.get_handle<vector<BoostedSingleTopHypothesis>>(rechyps_name);
  h_bstartotwgen = ctx.get_handle<BoostedSingleTopGen>(config.bstartotwgen_name);
}


bool BoostedSingleTopMatchDiscriminator::process(uhh2::Event & event){
  auto & hyps = event.get(h_hyps);
