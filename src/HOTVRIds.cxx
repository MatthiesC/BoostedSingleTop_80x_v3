...
bool DeltaPhiCut::operator()(const TopJet &topjet, const Event &event) const {
  if(event.muons->size() + event.electrons->size() < 1) return false;
  LorentzVector lepton;
  if (event.muons->size() == 1)
    {
      lepton = event.muons->at(0).v4();
    }
  else if (event.electrons->size() == 1)
    {
      lepton = event.electrons->at(0).v4();
    }
  return ( abs(deltaPhi(lepton, topjet.v4())) > m_deltaphi_lower );
}
