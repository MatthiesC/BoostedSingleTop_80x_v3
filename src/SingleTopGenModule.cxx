...
namespace uhh2examples {
   ...
   */
  class SingleTopGenModule: public AnalysisModule {
  public:
    
    explicit SingleTopGenModule(Context & ctx);
  ...
  SingleTopGenModule::SingleTopGenModule(Context & ctx){
    // In the constructor, the typical tasks are to initialize the
    // member variables, in particular the AnalysisModules such as
    // CommonModules or some cleaner module, Selections and Hists.
    // But you can do more and e.g. access the configuration, as shown below.
    
    cout << "Hello World from SingleTopGenModule!" << endl;
   ...
   // 3. Set up Hists classes:
    ...
    h_singletopgen.reset(new SingleTopGenHists(ctx, "singletopgen_nocuts"));
  }


  bool SingleTopGenModule::process(Event & event) {
    ...
    // cout << "SingleTopGenModule: Starting to process event (runid, eventid) = (" << event.run << ", " << event.event << "); weight = " << event.weight << endl;
  ...
  // make sure the SingleTopGenModule is found by class name. This is ensured by this macro:
  UHH2_REGISTER_ANALYSIS_MODULE(SingleTopGenModule)
